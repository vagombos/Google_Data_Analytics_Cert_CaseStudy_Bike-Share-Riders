...
all_trips_v2 %>% 
  ...
  filter(!is.na(member_casual)) %>% # Remove NA group
  arrange(member_casual, weekday) %>% 
  ggplot(aes(x = weekday, y = number_of_rides/1e6, fill = member_casual)) + # Convert to millions
  geom_col(position = "dodge") +
  scale_y_continuous(labels = scales::comma_format()) # Use commas for y-axis labels
